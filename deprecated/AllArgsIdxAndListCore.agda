...
module StreamGrids.ChoiceLog.AllArgsIdxAndListCore where
...
nfTransposed 
    {ℓ} {A} {_⊂_} {S}
    (i' , L' , root h') recurse q q'⋤q = Fin.zero
nfTransposed 
    {ℓ} {A} {_⊂_} {S}
...
nfTransposed 
    {ℓ} {A} {_⊂_}
    {record { card = ∞ ; idxToEl = idxToEl ; elToIdx = elToIdx 
              ; inv = inv ; subrelat = subrelat ; coerc = coerc }
    }
    q'@(i' , L' , choose q'' h'' 
...
nfTransposed 
    {ℓ} {A} {_⊂_}
    {record { card = card@(fin (suc n)) ; idxToEl = idxToEl ; elToIdx = elToIdx 
              ; inv = inv ; subrelat = subrelat ; coerc = coerc }
    }
    q'@(i' , L' , choose q'' h'' 
    lc''@(forcedChoice {i''} {L''} s'' h''' (ix , x⊂nextq'' , ix∉L') )) 
    ...
    let x⊂nextq''h'' = subst (λ v → (x ⊂ nextEl {q = q''} v)) (h'''≡h'') x⊂nextq'' in
    ...
    let ix<iq'-almost = subrelat x (el q') x⊂nextq''h'' in
    ...
    let Lx≼L' = multichoiceSuffix' {q' = qx} {q = q'} (inj₂ qx⋤q') in
    ...
    let ix'<ix = elimCaseLeft ix'≤ix ix'≢ix in
    let q* = coerc (nextEl h'') x x⊂nextq''h'' (idxToEl {! ix'-in-Lx !}) ? in
    ?
    where
        -- Existing indices in the enumeration of A.
        -- That's ℕ if A has infinitely many elements
